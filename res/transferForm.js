...
define([
	...
	var IniRequest = (function() {
		function addCustomProperty(param, handleInfo){
			if (!param['PROPERTY_LIST']) 
				param['PROPERTY_LIST'] = [];
		
			if (handleInfo.optionInfo.getParameter("PROPERTY_LIST")) {
				var cusPropertyList = handleInfo.optionInfo.getParameter("PROPERTY_LIST");
				for (var i = 0; i<cusPropertyList.length ; i++){
					var property = cusPropertyList[i];
					if(property["PROPERTY_KEY"] == "KEYBOARD_SECURITY_CLIENT") {
						param['PROPERTY_LIST'].push({
							"PROPERTY_KEY" :property['PROPERTY_KEY'] , "PROPERTY_VALUE" : property["PROPERTY_VALUE"] 
						});
					}
				}
			}
		...
		function constructJson(cmd, param, tranId, encrypted, sessionId, envelopedInfo){
			...
			if( envelopedInfo && envelopedInfo.isComplete ) {
			...
			if(param !== undefined){
				if(envelopedInfo && envelopedInfo.sessionSeed){
					/*
					여기서 undefined 확인되어서 위 조건에 추가함.
					if ( 'undefined' === typeof envelopedInfo.sessionSeed ) {
						console.log("Please try again in a few seconds.[#9]");
						throw new Error("Please try again in a few seconds.[#9]");
					}
					*/
...
//				var once_retrun = handleInfo.serviceInfo.getParameter(pwdType);
//				handleInfo.serviceInfo.setParameter(pwdType,"");
				if(handleInfo.serviceInfo.getParameter(pwdType)){
				...
				if("PWD" == pwdType){
					...
					return GINI_ProtectMgr.extract("NONCE_CNF");
				}else if("NEW_PWD" == pwdType){
					return GINI_ProtectMgr.extract("NEW_NONCE");
				}else if("NEW_PWD_CNF" == pwdType){
		...
		var getReqDeviceList = function(handleInfo, extTranId){
			...
			param["PROPERTY_LIST"] =
					...
					{"PROPERTY_KEY":"USIMSiteDomainURL", "PROPERTY_VALUE":"www.kjbank.com"},
                   	...
                   	{"PROPERTY_KEY":"certmanui_phone",   "PROPERTY_VALUE": defaultConf.Prop.get("certmanui_phone_prefix") + defaultConf.System.UrlAddress['URL'] + defaultConf.Prop.get("certmanui_phone")},
                   	{"PROPERTY_KEY":"certmanui_phoneURL","PROPERTY_VALUE": defaultConf.System.UrlAddress['URL'] + defaultConf.Prop.get("certmanui_phoneURL")},
                   	{"PROPERTY_KEY":"certmanui_phoneVer","PROPERTY_VALUE": defaultConf.Prop.get("certmanui_phoneVer")},
			...
			addCustomProperty(param, handleInfo);
			var sessionId = handleInfo.requestInfo.getParameter("SESSIONID");
			var encrypted = handleInfo.serviceInfo.getParameter("ENCRYPTED");
		...
		var getReqCertificateList = function(handleInfo, extTranId, envelopedInfo){
			...
			if ( pin && handleInfo.serviceInfo.getParameter("CERT_ID") )  {
				// 로그인인증서 정보가 있으면..			
				param["OPTION"]["CERT_CACHE"] = "TRUE";
			}
...
//			param["CONFIRM_PASSWORD"] = encodeURIComponent(handleInfo.serviceInfo.getParameter("PWD_CNF"));
			...
			param["CONFIRM_PASSWORD"] = encodeURIComponent(separateNonce(handleInfo, "NEW_PWD_CNF"));
...
//			param["PASSWORD"] = encodeURIComponent(handleInfo.serviceInfo.getParameter("PWD"));
//			param["CONFIRM_PASSWORD"] = encodeURIComponent(handleInfo.serviceInfo.getParameter("PWD_CNF"));
			param["PASSWORD"] = encodeURIComponent(separateNonce(handleInfo, "NEW_PWD"));
			param["CONFIRM_PASSWORD"] = encodeURIComponent(separateNonce(handleInfo, "NEW_PWD_CNF"));
			param["PROPERTY_LIST"] =
				[
//          			{"PROPERTY_KEY":"certmanui_language", "PROPERTY_VALUE":""},
...
//          			{"PROPERTY_KEY":"certmanui_language", "PROPERTY_VALUE":""},
		...
		var getReqSaveCertificate = function(handleInfo, extTranId, envelopedInfo){
			...
			if(handleInfo.serviceInfo.getParameter("PIN")){
				...
				if(handleInfo.serviceInfo.getDeviceId() === constants.Certs.STORAGE_SECURITY_DEVICE){
					param["PASSWORD"] = encodeURIComponent(separateNonce(handleInfo, "NEW_PWD"));
					param["CONFIRM_PASSWORD"] = encodeURIComponent(separateNonce(handleInfo, "NEW_PWD_CNF"));
				}else{
		 ...
		 * @desc : 인증서 요청 전문
		 */
		var getReqSaveCertificate12 = function(handleInfo, extTranId, envelopedInfo){
			var cmd = "SAVE_CERT";
			...
			param["CERT_P12"] = encodeURIComponent(encP12Cert);
			param["TYPE"] = encodeURIComponent("P12");
			var sessionId = handleInfo.requestInfo.getParameter("SESSIONID");
			var encrypted = handleInfo.serviceInfo.getParameter("ENCRYPTED");
		...
		var getReqGatterPCInfo = function(handleInfo, extTranId){
			...
			var param = {};
			// 사용고객 (CITIBANK / HANA / NONGHYUP / KJBANK)
		...
		return{
			...
			getReqSaveCertificate12 : getReqSaveCertificate12,
	...
	var IniResponse = (function() {
		...
		function extractParameters(jsonObj, envelopedInfo){
			...
			if(envelopedInfo){
				...
				if ( 'undefined' === typeof envelopedInfo.sessionSeed ) {
					console.log("Please try again in a few seconds.[#8]");
					throw new Error("Please try again in a few seconds.[#8]");
				}
				var sessionKey = envelopedInfo.sessionSeed.substring(0, 32);
				var sessionIv = envelopedInfo.sessionSeed.substring(32);
